import Header from "../../pages/header"
import { Link } from "react-router-dom"
import Footer from "../../pages/footer"

function Smart() {
    return(
 <div className="bg-gray-300">
<Header/>

<div className="flex justify-center  mt-28 space-x-32 pt-5 text-xl bg-gradient-to-r from-orange-500 to-transparent h-32 ">
    <div className="text-center"> <Link to="/phones"> Phones <img src="https://fscl01.fonpit.de/userfiles/7687254/image/Xiaomi_Smartphones_2023.jpg" className=" h-16 w-16 mt-2 rounded-full" /></Link></div>
    <div className=" text-center"> <Link to="/tvs"> TV-s <img src="data:image/jpeg;base64," className="h-16 w-16 mt-2 rounded-full" /> </Link> </div>
    <div className=" text-center"><Link to="/smart"> Smart Device <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSeEp71HheSyezxCMybJhMM94gj6Y0OUN1uqA" className=" h-16 w-16 mt-2 rounded-full ml-7" /></Link> </div> 
    <div className="  text-center"><Link to="/accessories"> Accessories <img src="https://english.onlinekhabar.com/wp-content/uploads/2022/07/Xiaomi-accessories-1.jpg" className="h-16 w-16 mt-2 rounded-full ml-5" /></Link> </div> 
 </div>

         <h1 className="text-5xl bg-gradient-to-r from-orange-500 to-transparent h-20 text-center mt-16 pt-3 ">Smart device</h1>

         <div className="flex flex-col space-y-[100px]  mt-32">
        
        <div className="grid grid-cols-4 gap-12 w-full  mt-16">

         <div className=" bg-gradient-to-r from-black to-gray-300 ml-14  w-[300px] h-[400px]  rounded-3xl  bg-white">
            <p className="text-center text-2xl  pt-6">Smart door lock</p>
            <p className="text-lg text-center">$ 250</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1673923841.7781291.png?width=400&height=400"  />
          
         </div>

            <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px]  rounded-3xl  bg-white">
            <p className="text-center text-2xl  pt-6">Automatic Door Lock</p>
            <p className="text-lg text-center">$220</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1673923636.67345556.png?width=400&height=400" alt="" />
         </div>

         <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px]  rounded-3xl  bg-white">
            <p  className="text-center text-2xl  pt-6"> Outdoor Camera AW300</p>
            <p className="text-lg text-center">$150</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1673868028.34594072.png?width=400&height=400" alt="" />
         </div>

         <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px]  rounded-3xl  bg-white">
            <p className="text-center text-2xl  pt-6"> Smart Speaker Lite</p>
            <p className="text-lg text-center">$70</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1673833596.47662450.png?width=400&height=400" alt="" />
         </div>
         </div>

<div className=" grid grid-cols-4 gap-12 w-full   ">
         <div className=" bg-gradient-to-r from-black to-gray-300 ml-14 w-[300px] h-[400px] rounded-3xl  bg-white">
            <p className="text-center text-2xl  pt-6"> Smart Camera C400</p>
            <p className="text-lg text-center">$200</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1672039614.53085652.png?width=400&height=400"/>
         </div>

         <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px] rounded-3xl   bg-white">
            <p className="text-center text-2xl  pt-6">Smart Camera C200</p>
            <p className="text-lg text-center">$100</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1668135658.08193372.png?width=400&height=400" alt="" />
         </div>
           

         <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px] rounded-3xl  bg-white">
            <p className="text-center text-2xl  pt-6">Speaker IR Control</p>
            <p className="text-lg text-center" >$99</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60  ml-7 mt-7"  src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666841971.73162728.png?width=400&height=400" alt="" />
         </div>

         <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px] rounded-3xl bg-white">
            <p className="text-center text-2xl  pt-6" >Smart Lightstrip</p>
            <p className="text-lg text-center">$40</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img  className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666872990.91731927.png?width=400&height=400" alt="" />
         </div>

         </div>


<div className=" grid grid-cols-4 gap-12 w-full  ">
         <div className=" bg-gradient-to-r from-black to-gray-300 ml-14 w-[300px] h-[400px] rounded-3xl bg-white">
            <p className="text-center text-2xl  pt-6">Smart Doorbell 3</p>
            <p className="text-lg text-center">$76</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60  ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666841374.53546011.png?width=400&height=400" alt="" />
         </div>

         <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px] rounded-3xl  bg-white">
            <p className="text-center text-2xl  pt-6">Window Sensor</p>
            <p className="text-lg text-center">$33</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7 " src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666842357.79238975.png?width=400&height=400" alt="" />
         </div>


         <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px] rounded-3xl  bg-white">
            <p className="text-center text-2xl  pt-6">360 Camera</p>
            <p className="text-lg text-center">$150</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666844622.74468703.jpg?width=400&height=400" alt="" />
         </div>

         <div className=" bg-gradient-to-r from-black to-gray-300 w-[300px] h-[400px] rounded-3xl  bg-white">
            <p className="text-center text-2xl  pt-6">Outdoor Camera</p>
            <p className="text-lg text-center">$129</p>
            <button className=" bg-gradient-to-r from-black to-transparent w-28 font-bold text-orange-600 rounded-md ml-24">Add to cart</button>
            <img className="w-60   ml-7 mt-7" src="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666844708.29229624.png?width=400&height=400" alt="" />
            </div>
         
         </div>
       
   <Footer/>
         </div>

 </div>

    )
}

export default Smart