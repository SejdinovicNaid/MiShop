import Header from "./header"
import { Link } from "react-router-dom"
import Footer from "./footer"
import PhonesData from "../Data/Products/PhonesData"

function TVs () {
    return(
 <div className="bg-gray-300">
<Header/>

<div className="grid grid-cols-5 gap-2  mt-28  pt-3 text-xl  bg-gradient-to-r from-gray-300 via-gray-400  to-gray-300 h-32 w-[65%] ml-[17.5%]">
    <h1>Cattegories:</h1>
   <div className=""> <Link to="/phones"> Phones <img src="https://fscl01.fonpit.de/userfiles/7687254/image/Xiaomi_Smartphones_2023.jpg" className=" h-16 w-16 mt-2 rounded-full hover:scale-110" /></Link></div>
   <div className=" "> <Link to="/tvs"> TV-s <img src="data:image/jpeg;base64," className="h-16 w-16 mt-2 rounded-full hover:scale-110 shadow-lg shadow-[#FF681F] scale-110" /> </Link> </div>
   <div className=" "><Link to="/smart">Smart Device <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSeEp71HheSyezxCMybJhMM94gj6Y0OUN1uqA" className=" h-16 w-16 mt-2 rounded-full ml-7 hover:scale-110" /></Link> </div> 
   <div className=" "><Link to="/accessories"> Accessories <img src="https://english.onlinekhabar.com/wp-content/uploads/2022/07/Xiaomi-accessories-1.jpg" className="h-16 w-16 mt-2 rounded-full ml-5 hover:scale-110" /></Link> </div> 
</div>

        

         <div className="'bg-gray-300 grid grid-cols-4  gap-10 place-content-evenly">

      

    <div className=''>
     <PhonesData 
      id=" dasfsfgfg"
     name="TV A Pro 43"
     price={250}
     button="Add to Cart"
     img=" https://i02.appmifile.com/960_operatorx_operatorx_opx/28/04/2023/1670e9f9914d0d6496a997fbf4af085a.png?width=400&height=400  "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" fsdfdsfd"
     name="TV A Pro 50"
     price={300}
     button="Add to Cart"
     img="  https://i02.appmifile.com/761_operatorx_operatorx_opx/28/04/2023/356659566c9d686590227eaad233f2d0.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" asdgfgg"
     name="TV A Pro 55"
     price={400}
     button="Add to Cart"
     img=" https://i02.appmifile.com/402_operatorx_operatorx_opx/28/04/2023/dcda6324db5fc91bbd3b15a729380f21.png?width=400&height=400  "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" adgdfgfg"
     name="TV F2 32"
     price={100}
     button="Add to Cart"
     img=" https://i02.appmifile.com/744_operatorx_operatorx_opx/17/04/2023/a094bafd71f26a5a7485a4b6751c5bc3.png?width=400&height=400  "
     />
    </div>

    <div className=''>
     <PhonesData 
      id="ghsvbghgfjn"
     name="TV Max 86"
     price={750}
     button="Add to Cart"
     img=" https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1669027257.44077078.png?width=400&height=400  "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" gdfsvsb"
     name="TV Q1E 55"
     price={700}
     button="Add to Cart"
     img="  https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1669027850.14511443.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" dsavvfgfh"
     name="TV P1E 43"
     price={400}
     button="Add to Cart"
     img="  https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666872191.83513416.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" agdfgfdg"
     name="TV Box S"
     price={100}
     button="Add to Cart"
     img="  https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1681282654.56587528.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" asgfgfgf"
     name="Soundbar 3.1 ch"
     price={200}
     button="Add to Cart"
     img=" https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666872294.81785792.png?width=400&height=400  "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" agfgfdgf"
     name="TV A2 FHD 43"
     price={420}
     button="Add to Cart"
     img="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666871973.311466.png?width=400&height=400   "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" agfgdfgdd"
     name="TV P1E 65"
     price={600}
     button="Add to Cart"
     img="  https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666871945.83744307.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
      id=" lafmkfjkhf"
     name="TV Q2 55"
     price={600}
     button="Add to Cart"
     img=" https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666871888.50679956.png?width=400&height=400  "
     />
    </div>



   
         </div>

        
     

       
       
   <Footer/>
         </div>


 

    )
}

export default TVs