import Header from "./header"
import { Link } from "react-router-dom"
import Footer from "./footer"
import PhonesData from "../Data/Products/PhonesData"

function Accessories() {
    return(
        <div className="bg-gray-300">
         <Header/>

         <div className="grid grid-cols-5 gap-2  mt-28  pt-3 text-xl  bg-gradient-to-r from-gray-300 via-gray-400  to-gray-300 h-32 w-[65%] ml-[17.5%]">
    <h1>Cattegories:</h1>
   <div className=""> <Link to="/phones"> Phones <img src="https://fscl01.fonpit.de/userfiles/7687254/image/Xiaomi_Smartphones_2023.jpg" className=" h-16 w-16 mt-2 rounded-full hover:scale-110" /></Link></div>
   <div className=" "> <Link to="/tvs"> TV-s <img src="data:image/jpeg;base64," className="h-16 w-16 mt-2 rounded-full hover:scale-110" /> </Link> </div>
   <div className=" "><Link to="/smart">Smart Device <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSeEp71HheSyezxCMybJhMM94gj6Y0OUN1uqA" className=" h-16 w-16 mt-2 rounded-full ml-7 hover:scale-110" /></Link> </div> 
   <div className=" "><Link to="/accessories"> Accessories <img src="https://english.onlinekhabar.com/wp-content/uploads/2022/07/Xiaomi-accessories-1.jpg" className="h-16 w-16 mt-2 rounded-full ml-5 hover:scale-110 shadow-lg shadow-[#FF681F] scale-110" /></Link> </div> 
</div>
       
        <div className="'bg-gray-300 grid grid-cols-4  gap-10 place-content-evenly">

        <div className=''>
     <PhonesData 
      id=" ačadkfjlkd"
     name="Writing Tablet"
     price={150}
     button="Add to Cart"
     img="  https://i02.appmifile.com/168_operatorx_operatorx_opx/14/04/2023/1fe83bef35f0b7819bcd10f691019c1c.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
    id=" adjfjsdlfj"
     name="Car Charger"
     price={10}
     button="Add to Cart"
     img=" https://i02.appmifile.com/786_operatorx_operatorx_opx/20/04/2023/98dcf715192eca63536f07026a6401d2.png?width=400&height=400  "
     />
    </div>

    <div className=''>
     <PhonesData 
     id="čadmnsvjk"
     name="Commuter Helmet"
     price={30}
     button="Add to Cart"
     img=" https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1667555627.31118619.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
     id="kačkdpč"
     name="Flashlight"
     price={17}
     button="Add to Cart"
     img=" https://i02.appmifile.com/524_operatorx_operatorx_opx/28/04/2023/01b79e9ea6f6268a7836b4c3fee38b13.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
     id="ldallldsdff"
     name="Smart Plug"
     price={23}
     button="Add to Cart"
     img="https://i02.appmifile.com/657_operatorx_operatorx_opx/18/05/2023/4da7716ff772d94c01ac74f1d93eb818.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
     id="dakljsdljfj"
     name="Laser Measure"
     price={99}
     button="Add to Cart"
     img="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1672036668.44359270.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
     id="addlčfksčkv"
     name="G11 Filter"
     price={10}
     button="Add to Cart"
     img="  https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666876995.7996159.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
     id = "klčdfmlsdl"
     name="Power Bank"
     price={30}
     button="Add to Cart"
     img="  https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666872499.8469504.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
     id= "lakpčcdkčv"
     name="Cable Lock"
     price={15}
     button="Add to Cart"
     img="  https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666872971.47157280.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
     id ="ldfdklnnjlns"
     name="Charging Combo"
     price={29}
     button="Add to Cart"
     img=" https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666872387.00972293.png?width=400&height=400  "
     />
    </div>

    <div className=''>
     <PhonesData 
     id = "amlkclnsdvj"
     name="Smart Clock"
     price={20}
     button="Add to Cart"
     img="https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666839084.21676720.png?width=400&height=400   "
     />
    </div>

    <div className=''>
     <PhonesData 
    id="alačclčksdv"
     name="Cordless Drill"
     price={69}
     button="Add to Cart"
     img=" https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666838191.21315054.png?width=400&height=400 "
     />
    </div>

    <div className=''>
     <PhonesData 
     id="akdlkjsdlf"
     name="Hair Clipper"
     price={60}
     button="Add to Cart"
     img=" https://i01.appmifile.com/v1/MI_18455B3E4DA706226CF7535A58E875F0267/pms_1666840245.99165624.png?width=400&height=400 "
     />
    </div>



   
         </div>

         
        <Footer/>
        </div>
    )
}

export default Accessories